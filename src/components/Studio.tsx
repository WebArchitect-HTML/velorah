      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        <div className="mb-20 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="reveal">
            <p className="mb-2 text-sm text-muted-foreground">Projets sélectionnés</p>
          ...
          {PROJECTS.map((project, i) => (
            <a key={project.title} href={project.href} target="_blank" rel="noreferrer" className={`reveal group block ${i % 2 === 1 ? 'md:mt-24' : ''}`} aria-label={`Ouvrir le projet ${project.title}`}>
              ...
              <div className="mt-6 flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-2xl font-normal text-foreground sm:text-3xl" style={{ fontFamily: "'Instrument Serif', serif" }}>{project.title}</h3>
                  <p className="mt-2 text-xs uppercase tracking-[0.2em] text-muted-foreground">{project.category}</p>
                </div>
